...
ALLOWED_CONTENT_TYPES = ["audio/mpeg", "audio/wav", "audio/x-wav", "audio/webm", "audio/mp4", "audio/aac", "audio/mp3"]
...
def validate_audio_upload(audio_file: UploadFile = File(...)) -> UploadFile:
    ...
    if audio_file.content_type not in ALLOWED_CONTENT_TYPES:
        logging.warning(f"File validation failed: unsupported content type: {audio_file.content_type}")
        raise HTTPException(
            status_code=415,
    ...
    if audio_file.size is not None and audio_file.size > MAX_AUDIO_BYTES:
        logger.warning(f"File validation failed: uploaded file is too large: {audio_file.size}")
        raise HTTPException(
            status_code=413,
            detail=f"Uploaded file is too large, (10MB maximum)"
        )
